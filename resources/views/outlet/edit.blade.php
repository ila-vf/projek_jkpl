        ...
        <div class="flex flex-wrap -mx-3">
            <div class="flex-none w-full max-w-full px-3">
                <div class="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 border-transparent border-solid shadow-xl dark:bg-slate-850 dark:shadow-dark-xl rounded-2xl bg-clip-border">
                    ...
                    <form action="{{ route('outlet.update',[$data->id]) }}" method="POST" class="flex-col justify-center p-6">
                        @method("PUT")
                        @csrf
                        ...
                        <div class="flex-col space-y-2 items-center w-full mb-5">
                            ...
                            <x-text-input id="hotline" class="block mt-1 w-full" type="text" name="hotline" :value="$data->hotline" required autofocus />
                            <x-input-error :messages="$errors->get('hotline')" class="mt-2" />
                        </div>
                        <div class="flex-col space-y-2 items-center w-full mb-5">
                            <x-input-label for="email" :value="__('Email')" />
                            <x-text-input id="email" class="block mt-1 w-full" type="text" name="email" :value="$data->email" required autofocus />
